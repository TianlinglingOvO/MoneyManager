...
SMB 2.3.0 使用数据库迁移版本 9，新增账户、别名、资金流水、转账、余额校准以及账目退款字段。不要手工改表；生产构建首次启动会先生成并验证迁移前快照，再自动补齐结构。升级只增加能力，不会自动启用资金追踪，也不会追溯或改写现有账目、事项、预算和操作历史。

### 首次启用资金追踪

迁移成功并不等于已经启用资金追踪。账本所有者需要在 SMB 的“资金”页面完成向导：保留或调整预设账户、填写启用当下的现实余额，并确认默认收入/支出账户。初始余额应已经包含启用前发生的资金变化；不要根据旧账再次扣减或补录。

当前 2.3.0 的启用边界只保存账本日期。若启用当天早些时候已经存在无账户账目，“账本体检”可能逐笔提示缺少账户。只有确实在启用后新增的账目才应补账户；已经计入初始现实余额的旧账应先标记已核对。详细说明及开发注意事项见 [资金追踪指南](FUNDS.md)。
...
探测成功后应看到原有账本、事项、AI、备份、时区和撤销工具，以及新增的 `list_accounts`、`get_funds_summary`、账户管理、转账、校准、退款和撤销退款工具。永久删除仍使用独立的显式确认工具。网页“OpenClaw 操作中心”可以选择：
...
- 在 1440×900、1024×768 和 412×915 下分别切换“分类明细/分类构成”：两个面板必须互斥；明细显示名次、金额和环比，构成完整显示全部有金额分类、金额和占比，且占比合计为 100%。
...
- 未主动建立普通账目关联时，借款和订阅不会改变洞察、月账单或 DeepSeek 消费分析。人民币关联金额必须一致，美元订阅创建账目时使用实际人民币扣款金额。
- 完整 JSON、借款、订阅、账户流水与转账 CSV 均可导出；恢复演练会核对资金表并确认临时写入、读回与清理后余额一致。
