...
探测成功后应看到 50 余个工具。除原有查询、提议和修订工具外，还包括借款人、独立借款、分次还款、订阅、付款历史、近期续费，以及账目、分类、AI、备份、时区、操作记录和撤销工具。永久删除另有 `get_category_deletion_impact`、`direct_permanently_delete_transaction` 和 `direct_permanently_delete_category` 三个专用工具。网页“OpenClaw 操作中心”可以选择：

- `confirm`：写入只生成待确认提案。
- `direct`：普通请求立即执行；只有明确要求“先让我确认”时才使用提案工具。

直接写入必须携带唯一 `requestId`；修改或删除还会校验最近查询到的更新时间。借款和订阅在 `confirm` 模式下只开放查询，在 `direct` 模式下才允许写入。可逆操作保留 30 天撤销期，且不会覆盖后来发生的新修改。永久删除必须显式调用专用工具并携带确认字段，分类还必须先读取最新影响版本并传回完整名称；这类操作不会生成恢复快照。密钥、Access 配置和服务启停永不开放给 OpenClaw。
...
- 完整 JSON、借款 CSV 与订阅 CSV 均可导出；回收站恢复和 OpenClaw 撤销不会覆盖后来发生的新修改。永久删除账目或分类后，当前库和新导出中不再包含对应账目，关联事项仍存在但账本关联已解除；旧备份会按原保留周期自然过期。
