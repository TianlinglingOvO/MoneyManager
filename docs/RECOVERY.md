...
6. 打开“资金”核对总额、账户和最近流水，再抽查借款、订阅、预算和账本体检。完整数据库恢复会同时恢复账户、别名、资金流水、转账、校准、退款状态、事项、预算、体检核对和 OpenClaw 的 30 天撤销快照；当前设备专属的个人背景图片仍只存在于浏览器 IndexedDB，不包含在 SQLite 备份中。
