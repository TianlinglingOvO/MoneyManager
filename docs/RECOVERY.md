...
不提供路径时会选择 `backups/local` 中最新的快照；也可把具体快照路径作为最后一个参数。演练会启动随机回环端口，核对账目、事项、预算、AI 报告和 OpenClaw 操作的数量与身份摘要，再在临时服务中创建、读回、软删除并永久清理一笔测试账目，最后重复执行完整性与外键检查。它不会替换正式数据库，也不会向正式账本写入测试数据；只会在正式库的 `settings` 中更新“恢复验证”的尝试、成功或失败状态。结束后只删除带 `smb-restore-drill-` 前缀的临时目录。
...
6. 打开“事项”，抽查一个借款人的未还余额和一项订阅的付款历史；再打开洞察页抽查当月预算和账本体检。完整数据库恢复会同时恢复借款人、借款、还款、订阅、付款、预算、体检核对状态、账本关联和 OpenClaw 的 30 天撤销快照；当前设备专属的个人背景图片仍只存在于浏览器 IndexedDB，不包含在 SQLite 备份中。
