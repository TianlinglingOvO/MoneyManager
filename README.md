...
一套只服务于单人的简体中文私人记账 PWA。它在 Windows 笔记本上运行，通过浏览器同时支持 Windows、Linux 和 Android；账本唯一数据源是本机 SQLite，浏览器不保存账目副本。
...
- DeepSeek 消费概览、异常增长、节省建议、分类结构和自定义问题；程序负责数字，模型只负责解释。
- 标准 Streamable HTTP MCP；可由网页在“确认模式”和“直接接管模式”间切换。直接模式可管理账目、分类、借款、订阅、AI、备份和时区，并保留 30 天可撤销操作记录。
- SQLite WAL、迁移前快照、每日完整性备份、`age` 公钥加密和 Google Drive 上传。
...
“事项”的实际操作、账本关联规则和 OpenClaw 权限见 [借款与订阅使用指南](docs/MATTERS.md)。
...
- 每个浏览会话首次冷启动会短暂显示一次主题自适应的“寸”字开屏；刷新数据和页面切换不会重播，系统开启“减少动画”时直接跳过。

## 本机预览

需要 Node.js 22 或更高版本。在项目目录打开 PowerShell：

最简单的方法是直接双击项目根目录的 `打开寸金记账.cmd`。它会启动本机服务并打开浏览器；请保留标题为“Money Manager Service”的窗口，关闭该窗口即停止服务。
