...
SMB 2.0.0 是 Sutady 的私人账本。它在 Windows 笔记本上运行，通过浏览器同时支持 Windows、Linux 和 Android；账本唯一数据源是本机 SQLite，浏览器不保存账目副本。
...
最简单的方法是直接双击项目根目录的 `打开 SMB.cmd`。它会启动本机服务并打开浏览器；请保留标题为“SMB Service”的窗口，关闭该窗口即停止服务。旧的 `打开寸金记账.cmd` 只作为已有快捷方式的兼容入口保留。
