...
- Funds tracking is opt-in. Never backfill or require accounts for records before `funds.started_on`; post-activation effective income/expense records require an active CNY account.
- SMB 2.3.0 stores the activation boundary as a ledger date only. Existing activation-day transactions can therefore be reported as missing an account even when their effect is already included in opening balances. Never “repair” those records by assigning an account without user confirmation. A future fix must persist an explicit activation baseline or creation cutoff and migrate safely; `localDate >= startedOn` alone is not sufficient.
